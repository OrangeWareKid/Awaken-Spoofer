...
namespace ImGui
{
	...
	NOTIFY_INLINE void RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			// Here we render the toast content
			{
				...
				GetForegroundDrawList()->AddRectFilled(pos + ImVec2(0, region.y), pos + ImVec2(opacity * region.x, region.y), GetColorU32(text_color), 10.f);
	...
	NOTIFY_INLINE void MergeIconsWithLatestFont(float font_size, bool FontDataOwnedByAtlas = false)
	{
		...
		GetIO().Fonts->AddFontFromMemoryTTF((void*)font_awesome, sizeof(font_awesome), font_size, &icons_config, icons_ranges);
	}
}
