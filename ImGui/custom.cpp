...
bool custom::tab2(const char* label, bool selected) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    static float line_pos = 0.f;
    line_pos = ImLerp(line_pos, selected ? bb.Min.y - window->Pos.y : line_pos, 0.04f);
    window->DrawList->AddRectFilled({ bb.Max.x - 2, window->Pos.y + line_pos }, { bb.Max.x, window->Pos.y + line_pos + size.y }, ImColor(0 / 255.f, 160 / 255.f, 255 / 255.f, anim), 2, ImDrawFlags_RoundCornersLeft);
...
bool custom::subtab(const char* label, bool selected) {
    ...
    window->DrawList->AddRectFilled({ bb.Min.x, bb.Max.y - 2 }, bb.Max, ImColor(0 / 255.f, 160 / 255.f, 255 / 255.f, anim), 2, ImDrawFlags_RoundCornersTop);
...
bool custom::collapse_button(bool collapsed) {
    ...
    ImRotateStart();
    RenderArrow(window->DrawList, bb.GetCenter() - ImVec2(2 + 2 * !anim, 6), ImColor(255, 0, 0), ImGuiDir_Down, 9.f);
